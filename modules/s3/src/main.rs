...
fn __abort_multipart_upload(input: AbortMultipartUploadRequest) -> BoxFuture<'static, Vec<u8>> {
    ...
    Box::pin(async move {
        match crate::CLIENT.call("DELETE", &path, input).await {
...
fn __complete_multipart_upload(input: CompleteMultipartUploadRequest) -> BoxFuture<'static, Vec<u8>> {
    ...
    Box::pin(async move {
        match crate::CLIENT.call("POST", &path, input).await {
            Ok(response) => serde_json::to_vec(&Result::<Vec<u8>, guest::Error>::Ok(response)).unwrap(),
            Err(why) => {
                serde_json::to_vec(&Result::<Vec<u8>, guest::Error>::Err(guest::Error {
                    why: why.to_string(),
...
fn __copy_object(input: CopyObjectRequest) -> BoxFuture<'static, Vec<u8>> {
    ...
    Box::pin(async move {
        match crate::CLIENT.call("PUT", &path, input).await {
            Ok(response) => serde_json::to_vec(&Result::<Vec<u8>, guest::Error>::Ok(response)).unwrap(),
            Err(why) => {
                serde_json::to_vec(&Result::<Vec<u8>, guest::Error>::Err(guest::Error {
                    why: why.to_string(),
...
fn __get_bucket_accelerate_configuration(input: GetBucketAccelerateConfigurationRequest) -> BoxFuture<'static, Vec<u8>> {
    ...
    Box::pin(async move {
        match crate::CLIENT.call("GET", &path, input).await {
            Ok(response) => serde_json::to_vec(&Result::<Vec<u8>, guest::Error>::Ok(response)).unwrap(),
            Err(why) => {
                serde_json::to_vec(&Result::<Vec<u8>, guest::Error>::Err(guest::Error {
                    why: why.to_string(),
...
fn __head_bucket(input: HeadBucketRequest) -> BoxFuture<'static, Vec<u8>> {
    ...
    Box::pin(async move {
        match crate::CLIENT.call("HEAD", &path, input).await {
            Ok(response) => serde_json::to_vec(&Result::<Vec<u8>, guest::Error>::Ok(response)).unwrap(),
            Err(why) => {
                serde_json::to_vec(&Result::<Vec<u8>, guest::Error>::Err(guest::Error {
                    why: why.to_string(),
